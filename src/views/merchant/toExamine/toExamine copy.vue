<template>
  <div id="details">
    <el-breadcrumb separator-class="el-icon-arrow-right" class="yetou">
      ...
      <el-breadcrumb-item :to="{ path: '/toExamine' }"
        >商户审核</el-breadcrumb-item
      >
    ...
    <el-form label-position="right" label-width="200px" :model="formLabelAlign">
      ...
      <el-form-item class="elFor" label="银行账户：">
        <el-input disabled v-model="formLabelAlign.bankAccountFile"></el-input>
      </el-form-item>
    ...
    <Dialog :dialogForm="dialogForm" :disabledbo="disabledbo" />
...
export default {
  ...
  methods: {
    ...
    detail(clientId) {
      examinedetail(clientId).then((res) => {
        if (res.data.code == "1000") {
          ...
          let _rates = res.data.result.rates ? res.data.result.rates : [];
          for (let i = 0; i < _rates.length; i++) {
            if (_rates[i].payTypeID == "1") {
              this.dialogForm.WalletForm = _rates[i];
            } else if (_rates[i].payTypeID == "2") {
              this.dialogForm.UPIForm = _rates[i];
            } else if (_rates[i].payTypeID == "3") {
              this.dialogForm.DebitForm = _rates[i];
            } else if (_rates[i].payTypeID == "4") {
              this.dialogForm.CreditForm = _rates[i];
            }
          }
        } else {
    ...
    _merchantpass() {
      let datas = {
        mid: this.information.clientId,
        countryCode: "IN",
        currency: "INR",
        bankCardNumber: "424316625234453653",
        rateList: [
          this.dialogForm.WalletForm,
          this.dialogForm.UPIForm,
          this.dialogForm.DebitForm,
          this.dialogForm.CreditForm,
        ],
      };
      merchantpass(datas).then();
    },
  },
  created() {
